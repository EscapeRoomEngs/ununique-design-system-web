        ...
        {Object.keys(usageObj).map((styleNm, index) => {
          ...
          return (
            <tr key={index}>
              ...
              <td className="uui-legacy-weight-600">{colorObj.styleNm}</td>
...
Waiting RN의 확정 기준값은 `orange100` (`#FFF2E9`), `orange300` (`#FB974B`), `orange500` (`#FF3D00`)이며, pressed 상태는 `orange600` (`#E63600`)입니다. 나머지 단계는 이 기준값을 연결해 Web 디자인 시스템용으로 정의한 확장 팔레트입니다.
...
<ColorUsageTable
  ...
  getStyleExample={(styleNm, hex) => (
    <div
      className="uui-legacy-weight-600"
      style={{
