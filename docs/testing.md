...
Storybook-only compatibility helpers must use the `uui-legacy-*` namespace. Do not add generic selectors such as `.grid`, `.text-*` or `.border-*`: they can override Tailwind utilities in rendered component stories. A Storybook reset must be contained in the `base` cascade layer, declared before Tailwind `utilities`; an unlayered reset can override component padding, borders and typography. Class assertions protect source mappings, but visual approval still requires reviewing the rendered parity stories at the target viewport.
